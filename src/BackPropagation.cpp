...
double antsigmoid (double y)
{
	return -log(1.0/y - 1.0);
}

BackPropagation::BackPropagation(int nInput, int nHide, int nOutput)
{
	_nInput = nInput;
	_nHide = nHide;
	_nOutput = nOutput;
...
void BackPropagation::Classify(double plfInput[], double plfOutput[])
{
...
bool BackPropagation::Train(int n, double **pplfInput, double **pplfDesire)
{
	const double a = 0.1;
	const double E = 0.01;
	double lfE = E + 1;

	int iterDepth = 100, count = 0;
